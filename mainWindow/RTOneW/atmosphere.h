...
public:
	...
	vec3 computeIncidentLight(const vec3& orig, const vec3& dir, float tmin, float tmax) const;
...
bool atmosphere::bounding_box(float t0, float t1, aabb& box) const {
...
vec3 atmosphere::computeIncidentLight(const vec3& orig, const vec3& dir, float tmin, float tmax) const
{
	float t0, t1;
