...
__global__ void init_fb(vec3* fb, int max_x, int max_y) {
...
__global__ void create_world(hitable **d_list, hitable **d_world, camera **d_camera, int nx, int ny,float fov,float aperture, curandState *rand_state) {
	if (threadIdx.x == 0 && blockIdx.x == 0) {
...
void render(int width, int height, int spp, float fov, float aperture) {
	...
	create_world << <1, 1 >> > (d_list, d_world, d_camera, nx, ny,fov,aperture, d_rand_state2);
	checkCudaErrors(cudaGetLastError());
	checkCudaErrors(cudaDeviceSynchronize());
